...
export default function ImagesPage() {
  ...
  const selectedTypes = ['image']; // Fixed to image type
  ...
  // Event handler for image drop on cards
  useEffect(() => {
  ...
  const applyFiltersAndSort = (
    ...
    // Filter by type (always image for this page)
    if (types.length > 0) {
  ...
  const handleFilterChange = async (filters: { tags?: string[] }) => {
    ...
    try {
      ...
      // Fetch filtered image cards from server
      ...
      const sorted = applyFiltersAndSort(response.cards, ['image'], tags, currentSort, '');
      setFilteredCards(sorted);
  ...
  const handleSortChange = (sortOption: SortOption) => {
    ...
    const filteredAndSorted = applyFiltersAndSort(cards, ['image'], selectedTags, sortOption, searchTerm);
  ...
  const handleSearch = async (search: string) => {
    ...
    try {
      ...
      const sorted = applyFiltersAndSort(response.cards, ['image'], selectedTags, currentSort, '');
      setFilteredCards(sorted);
      ...
      if (response.cards.length < response.totalCount) {
        setTimeout(async () => {
          try {
            ...
            const newSorted = applyFiltersAndSort(combinedCards, ['image'], selectedTags, currentSort, '');
            setCards(combinedCards);
      ...
      <header className="bg-[#d9f2fc] border-b border-gray-200 sticky top-0 z-40">
        <div className="max-w-7xl mx-auto px-4 py-3">
          ...
          <div className="flex justify-between items-center relative">
            ...
            <h1 className="text-xl font-bold text-gray-900 hidden sm:block">Image Resources</h1>
          </div>
      ...
      <main className="max-w-7xl mx-auto px-4 pt-6">
        {!loading && totalCardCount > 0 && (
          <div className="mb-4 text-gray-600 text-sm">
            Showing {filteredCards.length} of {totalCardCount} image cards
          </div>
        )}
