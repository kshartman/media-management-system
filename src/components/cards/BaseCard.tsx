    ...
    <div className="bg-white rounded-lg shadow-md overflow-hidden flex flex-col h-full">
      ...
      <div className="border-t border-gray-200 p-4 bg-gray-50 flex justify-between items-center">
        <div className="flex items-center space-x-3">
          ...
          {/* Social Copy Icons - conditionally show based on content and admin status */}
          {/* Instagram button */}
          {(isAdmin || instagramCopy) && (
            <button 
              onClick={() => handleOpenSocialCopy('instagram')}
              className={`p-1.5 rounded-full hover:bg-gray-100 transition-colors ${!instagramCopy && isAdmin ? 'opacity-50' : ''}`}
              title={instagramCopy ? "View Instagram Copy" : "Add Instagram Copy"}
              aria-label={instagramCopy ? "View Instagram Copy" : "Add Instagram Copy"}
            >
          ...
          {/* Facebook button */}
          {(isAdmin || facebookCopy) && (
            <button 
              onClick={() => handleOpenSocialCopy('facebook')}
              className={`p-1.5 rounded-full hover:bg-gray-100 transition-colors ${!facebookCopy && isAdmin ? 'opacity-50' : ''}`}
              title={facebookCopy ? "View Facebook Copy" : "Add Facebook Copy"}
              aria-label={facebookCopy ? "View Facebook Copy" : "Add Facebook Copy"}
            >
