...
import { useSocialSlide } from '../../contexts/SocialSlideContext';
...
const SocialCard: React.FC<SocialCardProps> = (props) => {
  ...
  const slideId = `slide-${props.id}`; // Unique ID for this slideshow
  
  // Get slideshow context
  const { playSocialSlide, isPlaying, stopAllSlides } = useSocialSlide();
  ...
  const getCurrentImage = () => {
    // If no images in sequence or empty array, use placeholder
    if (!props.imageSequence || props.imageSequence.length === 0) {
      return null;
    }
    return props.imageSequence[currentImageIndex];
  };
  
  // Get display image (preview or first in sequence)
  const getDisplayImage = () => {
    if (isPlaying(slideId)) {
  ...
  // Sync lightbox state with play state
  useEffect(() => {
    // Don't automatically close the lightbox when the slideshow stops
    // as we want to control this manually
  }, [isPlaying, slideId, lightboxOpen]);
  
  // We don't need to handle cycling through images anymore
  // since the Lightbox component handles this internally
  useEffect(() => {
    // Cleanup any existing timer when the component unmounts
    return () => {
      if (timerRef.current) {
  ...
  const handlePlay = () => {
    // Set lightbox open first to ensure it's visible
    setLightboxOpen(true);
    // Then stop any playing slideshows after a small delay
    setTimeout(() => {
    ...
    <BaseCard {...baseProps} preview={props.preview} transcript={props.transcript} imageSequence={props.imageSequence}>
      ...
      <div className="relative group">
        <div 
          ...
          onClick={() => setLightboxOpen(true)} // Directly open lightbox on click
          ...
          onKeyDown={(e) => {
            if (e.key === 'Enter' || e.key === ' ') {
              setLightboxOpen(true); // Directly open lightbox
            }
          }}
        ...
        >
          ...
          {/* Expand, Document Download, and Transcript Icon Overlays (only show when not playing) */}
          {!isPlaying(slideId) && (
